...
public:
    ...
    double sampleThirdAbsMoment;

    SampleEstimates() : sampleSize(0), sampleMean(0), sampleVariance(0), sampleThirdAbsMoment(0) {}

    friend std::ostream &operator<<(std::ostream &os,
                                    const SampleEstimates &est) {
        os << est.sampleSize << "\t" << est.sampleMean
           << "\t" << est.sampleVariance << "\t" << est.sampleThirdAbsMoment;
        return os;
    }
};
...
SampleEstimates estimatesFromSamples(T samples[], size_t k) {
    // TODO: Use function below for iterator ...(samples[0], samples[k-1], k)
    SampleEstimates est;
    est.sampleSize = k;
    ...
    // estimates the 'sample variance' and 'sample skeweness'
    double tmp = 0;
    for (size_t i = 0; i < est.sampleSize; ++i) {
        tmp = (samples[i] - est.sampleMean);
        est.sampleVariance += tmp * tmp / (double) (est.sampleSize - 1);
        est.sampleThirdAbsMoment += std::abs(tmp * tmp * tmp) / (double) (est.sampleSize - 1);
    }
    est.sampleThirdAbsMoment /= std::pow(est.sampleVariance, 3.0 / 2.0);
